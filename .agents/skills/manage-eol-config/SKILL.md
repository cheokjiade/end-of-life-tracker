...
- For a new config sourced only from clean `pom.xml`, `*.gradle*`, or `package.json` files, run `generate_config.py <folder> --name <project>`. Review `_skipped_npm_packages`; the generated file is a draft until verified.
